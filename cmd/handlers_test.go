...
func TestBuildFilterInfo(t *testing.T) {
	...
	if !strings.Contains(got, "www.example.com") {
		t.Errorf("应包含 www.example.com, got %q", got)
	}
	if !strings.Contains(got, "example.com") {
		t.Errorf("应包含 example.com, got %q", got)
	}
...
func TestHandleList_EmptyAndFound(t *testing.T) {
	...
	t.Run("empty", func(t *testing.T) {
		...
		out := captureStdout(t, func() {
			if err := handleList(cmd, domains, &mockDNS{}); err != nil {
				t.Fatalf("handleList: %v", err)
			}
		})
		if !strings.Contains(out, "No records found") {
			t.Errorf("期望无记录提示, got %q", out)
		}
	})
	...
	t.Run("found", func(t *testing.T) {
		...
		if !strings.Contains(out, "Found 1") {
			t.Errorf("期望找到记录, got %q", out)
		}
		if !strings.Contains(out, "filtered by subdomain") {
			t.Errorf("显式 subdomain 应标注过滤, got %q", out)
		}
	})

	t.Run("query error", func(t *testing.T) {
		cmd := listCleanFlags(t)
		err := handleList(cmd, domains, &mockDNS{getErr: errors.New("boom")})
		if err == nil {
			t.Fatal("查询失败应返回错误")
		}
	})
...
// TestHandleClean 覆盖 dry-run、--yes 删除与删除失败。
func TestHandleClean(t *testing.T) {
	...
	t.Run("no records", func(t *testing.T) {
		cmd := listCleanFlags(t)
		out := captureStdout(t, func() {
			if err := handleClean(cmd, domains, &mockDNS{}); err != nil {
				t.Fatalf("handleClean: %v", err)
			}
		})
		if !strings.Contains(out, "No matching records") {
			t.Errorf("got %q", out)
		}
	})
	...
	t.Run("yes delete", func(t *testing.T) {
		...
		if len(m.deleted) != 1 {
			t.Fatalf("应删除 1 条, got %d", len(m.deleted))
		}
	...
	t.Run("query error", func(t *testing.T) {
		cmd := listCleanFlags(t)
		err := handleClean(cmd, domains, &mockDNS{getErr: errors.New("boom")})
		if err == nil {
			t.Fatal("查询失败应返回错误")
		}
	})
...
// TestHandleClean_CancelOnConfirm 验证交互确认拒绝时取消删除。
func TestHandleClean_CancelOnConfirm(t *testing.T) {
	domains := []*ddns.Domain{{Domain: "example.com", SubDomain: "www", Type: "AAAA"}}
	cmd := listCleanFlags(t)
	m := &mockDNS{records: []ddns.RecordInfo{
		{ID: "1", Name: "www.example.com", Type: "AAAA", Value: "2001:db8::1"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !strings.Contains(out, tt.substr) {
				t.Errorf("期望包含 %q, got %q", tt.substr, out)
			}
		})
	}
}
...
func TestCheckFromConfig_APIPath(t *testing.T) {
	...
	if !strings.Contains(out, "Provider 'cloudflare' is valid") {
		t.Errorf("期望 provider 校验通过, got %q", out)
	}
	if !strings.Contains(out, "API Connectivity Test") {
		t.Errorf("期望进入 API 测试, got %q", out)
	}
}
...
func TestRequireFlags_InvalidFlag(t *testing.T) {
	cmd := &cobra.Command{}
	err := requireFlags(cmd, []providerFlag{{name: "missing-flag"}})
	if err == nil {
...
func TestRunWithConfig_LoadError(t *testing.T) {
	...
	cmd := &cobra.Command{}
	err := runWithConfig(cmd, "list", func(*cobra.Command, *config.Config, []*ddns.Domain, ddns.DNSProvider) error {
...
`)
	called := false
	cmd := &cobra.Command{}
	err := runWithConfig(cmd, "list", func(_ *cobra.Command, cfg *config.Config, domains []*ddns.Domain, p ddns.DNSProvider) error {
		called = true
...
`)
	cmd := listCleanFlags(t)
	if err := runListWithConfig(cmd); err == nil || !strings.Contains(err.Error(), "does not support 'list'") {
		t.Fatalf("受限 list 错误不符: %v", err)
	}
	if err := runCleanWithConfig(cmd); err == nil || !strings.Contains(err.Error(), "does not support 'clean'") {
...
func TestRunServiceFromConfigHandler(t *testing.T) {
	...
	var gotIface string
	serviceRunner = func(_ []*ddns.Domain, _ ddns.DNSProvider, interval time.Duration, _ []ipaddr.IPv6Fetcher, iface string) error {
	...
	t.Run("invalid interval", func(t *testing.T) {
		cfg := &config.Config{Interval: "bad"}
		err := runServiceFromConfigHandler(nil, cfg, nil, &mockDNS{})
		if err == nil {
